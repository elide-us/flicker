...
fn load_reference_skeleton(path: &Path) -> Result<Vec<RefBone>> {
    let text = std::fs::read_to_string(path).with_context(|| format!("reading reference {}", path.display()))?;
...
pub fn default_reference() -> std::path::PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../Alpha/content/characters/PrismHumanBaseA/PrismHumanBaseA.json")
}
...
mod tests {
    ...
    fn low_res_human_conforms_sanely() {
        let low = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../Alpha/content/source/PrismRaces/HumanBaseA_Low");
        let reference = default_reference();
    ...
    fn diagnose_hip_geometry() {
        ...
        for (label, rel) in bodies {
            let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../Alpha/content/source").join(rel);
    ...
    fn diagnose_shoulder_geometry() {
        let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../Alpha/content/source/PrismRaces/HumanBaseA_Low");
        let Some(fbx) = dir.exists().then_some(()).and_then(|_| {
            std::fs::read_dir(&dir).ok()?.filter_map(|e| e.ok().map(|e| e.path())).find(|p| {
                p.to_string_lossy().contains("Character_output") && p.extension().map(|e| e == "fbx").unwrap_or(false)
            })
    ...
    fn idle_pose_shoulder_effect() {
        ...
        let posed_world = |clip: &Path| -> (Vec<Mat4>, HashMap<String, usize>) {
            ...
            let idx: HashMap<String, usize> =
                refs.iter().enumerate().map(|(i, b)| (b.name.clone(), i)).collect();
            let v: serde_json::Value =
                serde_json::from_str(&std::fs::read_to_string(clip).unwrap()).unwrap();
