...
pub fn import_folder(source_dir: &Path, out_dir: &Path, asset_name: &str, reference: &Path) -> Result<ImportSummary> {
    ...
    std::fs::write(&rig_path, json).with_context(|| format!("writing {}", rig_path.display()))?;
...
mod tests {
    ...
    fn humanbasea_loads_in_engine_with_clips() {
        ...
        if !base.join("HumanBaseA.json").exists() {
    ...
    fn humanbasea_rest_skin_matches_bind_mesh() {
        ...
        let base = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../Alpha/content/characters/HumanBaseA");
        if !base.join("HumanBaseA.json").exists() {
