...
fn load_target(skeleton: &Path) -> Result<Target> {
    let text = std::fs::read_to_string(skeleton).with_context(|| format!("reading target {}", skeleton.display()))?;
...
pub fn emit_variants(bvh_path: &Path, skeleton: &Path, out_dir: &Path) -> Result<(std::path::PathBuf, std::path::PathBuf)> {
    ...
    std::fs::write(&ip_path, serde_json::to_string(&ip)?)?;
    std::fs::write(&rm_path, serde_json::to_string(&rm)?)?;
    Ok((ip_path, rm_path))
}
...
mod tests {
    ...
    fn retarget_reproduces_python_oracle() {
        let content = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../Alpha/content");
        let bvh = content.join("source/Motifect/Motifect_locomotion_complete_v1_0/BVH/walk_forward.bvh");
        let skeleton = content.join("characters/PrismHumanBaseA/PrismHumanBaseA.json");
        let oracle = content.join("retarget/clips/locomotion/RootMotion/walk_forward.json");
        ...
        let load = |p: &Path| -> Value { serde_json::from_str(&std::fs::read_to_string(p).unwrap()).unwrap() };
