//! Orbit camera: drag to rotate around the planet, wheel to zoom. (The shared
//! planet-viewer camera, matching the other globe POCs.)
...
impl OrbitCam {
    ...
    /// Apply this frame's input: left-drag rotates, wheel zooms. `allow_rotate` is
    /// `false` while the pointer is over the HUD, so clicking a widget doesn't also
    /// spin the planet (wheel-zoom still works).
