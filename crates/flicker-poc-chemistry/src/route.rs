//! The chemistry scene's input-router handler chain (spec §9), replacing the
//! hand-rolled `prev_menu` edge with the shared event bus (the same one
//! `flicker-pocclusters` is ported onto).
//!
//! Chemistry is a Group-B raw-key data viewer: only `Menu` (Esc -> the shell pause
//! overlay) rides the bus. The orbit camera (left-drag rotate, wheel zoom) and the
//! bespoke discrete keys (Space play/pause, Down reset, V mantle view, R reseed)
//! pick / poll straight off the RAW snapshot, so that logic is unchanged. The HUD
//! is a display-only walker tree (readout + conservation ledger), so the chain is
//! two layers:
//!
//! ```text
...
//!                        `on_menu = "pause_open"` on the hud_chemistry.lua
...
/// DATA on the screen root now (`on_menu = "pause_open"` in `hud_chemistry.lua`),
