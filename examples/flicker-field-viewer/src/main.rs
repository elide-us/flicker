//! flicker-field-viewer — the instanced GPU-skinning **field viewer**.
//!
...
//!
//! Reuses `examples/flicker-animation/assets` (the Katanami rig + 91-clip library + the
//! `Katanami.pack.json` state graph) — no asset duplication.
//!
...
/// pipeline. Same helper as `flicker-animation`.
...
/// grid centre (the origin). Mirrors `flicker-animation`'s `OrbitCam`, retuned for a
...
impl Field {
    ...
    /// blend), mirroring `flicker-animation`'s Graph mode. `None` if no state machine.
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                "flicker_field_viewer=info,flicker_app=info,flicker_render=warn".into()
            }),
        )
        .init();

    // Reuse the single-character viewer's assets (rig + 91-clip library + state pack).
    let assets = PathBuf::from(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../flicker-animation/assets"
    ));
