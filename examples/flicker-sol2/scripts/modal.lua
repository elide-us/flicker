-- Reusable flat modal: a centred panel with a title (+ optional subtitle) and a
-- vertical stack of buttons, driven by ui_elements.json `modal` (style) +
-- `screens[Model.screen]` (overlay/title/subtitle/items). Shared by the menu /
-- pause / settings screens. Returns `{ <item id> = clicked }`; the host turns
-- those into scene transitions. Plain Lua.
...
function M.draw(sw, sh)
  ...
  -- Subtitle: prefer the screen's own static line, else a live `Model.subtitle`.
  local subtitle = (sc and sc.subtitle) or Model.subtitle
  if subtitle and subtitle ~= "" then
    local st = m.subtitle
    cmds[#cmds + 1] = {
      kind = "text", x = px + m.panel.w * 0.5, y = py + st.offset_y,
      text = subtitle, size = st.size, align = "center",
