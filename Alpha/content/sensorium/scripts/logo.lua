-- Intro splash: a sequence of full-screen logos, each fading in, holding, then
-- fading out, before the menu. Driven by ui_elements.json (`UI.logo`) + the
-- engine `Model` (elapsed seconds + each image's pixel size). Edit UI.logo to
-- change the images, timing, fit, or backdrop — no recompile. Plain Lua.
--
--   update(...) -> { done = bool }   -- true once the whole sequence has played
--   draw(sw, sh) -> { backdrop, <current image faded> }
...
function M.draw(sw, sh)
  ...
  if tex then
    -- The active image's native size comes from the Model (img<idx>_w/h);
    -- fit it inside `fit` of the screen, preserving aspect, and centre it.
