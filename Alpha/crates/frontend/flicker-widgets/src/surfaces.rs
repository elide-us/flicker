...
mod tests {
    ...
    /// A Screen declaration naming an rtt-view surface round-trips visibility like
    /// any other surface: hidden, the walker never places the subtree, so its
    /// `RttSlot`s are not reserved; shown, both stages come back. This is the S8
    ...
    #[test]
    fn an_rtt_view_surface_round_trips_visibility() {
        let json = r#"{ "component": "screen", "children": [
            { "component": "row", "id": "pair", "visible_bind": "workbench_on", "gap": 4,
              "children": [
                { "component": "rtt", "id": "pair_left_rtt",  "tab_group": "l", "source": "cam_a" },
                { "component": "rtt", "id": "pair_right_rtt", "tab_group": "r", "source": "cam_b" }
              ] }
        ] }"#;
