...
struct PerDraw {
    model: [[f32; 4]; 4],
    tint: [f32; 4],
    flags: [f32; 4], // flags.y = gloss (sheen strength); rest reserved.
}
...
impl TexturedMeshPipeline {
    pub fn new(
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(include_str!("shaders/mesh_textured.wgsl").into()),
        });
        ...
        // Combined material layout: 5 textures (albedo, normal, roughness, metalness, AO)
        // at bindings 0–4 + one shared sampler at binding 5. Packing the whole material
        // into ONE bind group keeps the pipeline within the default `max_bind_groups`
        // limit of 4 (group 0 = uniforms, group 1 = material).
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            ...
            bind_group_layouts: &[&uniform_bgl, &material_bgl],
        ...
        let camera_buf = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("flicker.mesh_textured.camera_uniform"),
            size: CAMERA_UNIFORM_SIZE,
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
        let scene_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("flicker.mesh_textured.scene_uniform"),
        ...
        let uniform_bind_group =
            make_uniform_bind_group(device, &uniform_bgl, &camera_buf, &scene_buf, &per_draw_buf);
    ...
    fn textured_mesh_pipeline_compiles_shader() {
        ...
        let (device, queue) = pollster::block_on(adapter.request_device(
            &wgpu::DeviceDescriptor {
                label: Some("flicker.mesh_textured_test.device"),
        ...
        let mut pipeline =
            TexturedMeshPipeline::new(&device, &queue, wgpu::TextureFormat::Rgba8UnormSrgb, align);
