...
//! range `[-0.5, 1.5]` to `[0, 255]`. Encoding and decoding are lossy round
//! trips (max axis error ≈ `1/255` ≈ `0.0039`), so equality is defined on
//! the stored byte representation rather than on decoded floats — two
//! `CornerVector`s are equal iff their three bytes match.
//!
...
//! voxels in later phases.

/// Byte value that encodes the axis default `0.5`.
///
/// `((0.5 + 0.5) / 2.0 * 255.0).round() == 128`.
const DEFAULT_AXIS_BYTE: u8 = 128;
...
const AXIS_MAX: f32 = 1.5;

/// A per-voxel corner offset stored as three bytes — one per axis.
///
/// Constructible from `f32` components (with clamping to `[-0.5, 1.5]` and
...
impl CornerVector {
    /// The default corner vector `(0.5, 0.5, 0.5)`, encoded as `[128, 128, 128]`.
    ...
    /// clamped to `[-0.5, 1.5]`; non-finite inputs (NaN, ±Inf) fall back
    ...
    /// Decode the three axis components as `f32`s in `[-0.5, 1.5]`.
    #[inline]
    #[must_use]
...
/// Decode one axis byte back to its `f32` value in `[-0.5, 1.5]`.
#[inline]
fn decode_axis(byte: u8) -> f32 {
    (byte as f32 / 255.0) * 2.0 - 0.5
}
...
mod tests {
    ...
    fn default_is_centered() {
        ...
        assert_eq!(v.bytes(), [128, 128, 128]);
        let [x, y, z] = v.to_components();
        // Default encoding 128 decodes to ~0.5039, not exactly 0.5.
    ...
    fn extremes_encode_to_byte_endpoints() {
        ...
        assert_eq!(
            CornerVector::from_components(1.5, 1.5, 1.5).bytes(),
            [255, 255, 255]
        );
    }
    ...
    fn out_of_range_inputs_clamp() {
        ...
        assert_eq!(
            CornerVector::from_components(2.0, 100.0, 1.500001).bytes()[0],
            255
        );
        assert_eq!(
            CornerVector::from_components(2.0, 100.0, 1.500001).bytes()[1],
            255
        );
        assert_eq!(
            CornerVector::from_components(2.0, 100.0, 1.500001).bytes()[2],
            255
        );
    }
    ...
    fn round_trip_precision_within_one_step() {
        ...
        // within 1/255 of the original. This is the documented precision
        // bound and the practical accuracy a caller can rely on.
        let step = 0.01_f32;
        let mut v = -0.5_f32;
        while v <= 1.5 {
    ...
    fn equality_is_on_bytes_not_decoded_floats() {
        ...
        // 0.282 → 99.7→100, 0.286 → 100.2→100.
        let a = CornerVector::from_components(0.282, 0.282, 0.282);
        let b = CornerVector::from_components(0.286, 0.286, 0.286);
    ...
    fn bytes_constructor_round_trips() {
        for &b in &[0u8, 1, 42, 127, 128, 200, 254, 255] {
