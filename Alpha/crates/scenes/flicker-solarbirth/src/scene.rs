...
const HUD_SCRIPT: &str = concat!(
    ...
    "/../../../content/sensorium/scripts/hud_solarbirth.lua"
);
...
impl Sim {
    ...
    fn hud_model(&self) -> ValueMap {
        ...
        // Static copy rides stringtable tokens (Model-channel strings gate); the
        // segment name + progress are data composing around the resolved text.
        let sys = strings::resolve("$sb_the_prism_system");
        let phase = if self.flight.progress() >= 1.0 {
            format!("{sys} · {seg} · {}", strings::resolve("$sb_settled"))
        } else {
            format!(
                "{sys} · {seg} · {} {:.0}%",
                strings::resolve("$sb_approaching"),
                self.flight.progress() * 100.0,
            )
        };
