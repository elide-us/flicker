//! The God Mode scene's input-router handler chain (spec §9), replacing the
//! hand-rolled `prev_menu` edge with the shared event bus (the same one
//! `flicker-pocclusters` is ported onto).
//!
//! This is a Group-B raw-key data viewer: only `Menu` (Esc -> the shell pause
//! overlay) rides the bus. The orbit camera (left-drag rotate, wheel zoom) and the
//! bespoke discrete keys (Space play/pause, Down reset, V mantle view, C cutaway,
//! R reseed) pick / poll straight off the RAW snapshot. The HUD
...
//!                        `on_menu = "pause_open"` on the screen root
...
/// DATA on the screen root now (`on_menu = "pause_open"`, declared in
/// `GodModeScene::build_tree`),
