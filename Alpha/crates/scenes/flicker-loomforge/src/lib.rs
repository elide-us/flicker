...
impl LoomforgeBench {
    ...
    /// Rebuilt every frame NOT because the walker needs it (its draw cache is retained)
    /// but because the bench mutates its document mid-frame and node ids
    /// (`packcard_<i>` / `packrow_<i>` / …) encode positions in the FILTERED library —
    /// they must be re-derived against the post-mutation state. The rebuild costs
    /// nothing on quiet frames: cache identity is structural, so an equal tree replays
    /// (`a_rebuilt_tree_still_hits_the_cache` in flicker-widgets' component tests).
...
mod tests {
    ...
    /// shape, or carry an explicit `strings-gate-exempt` reason. The bench's
    /// publishes are all data today; this keeps future drift a build failure.
    #[test]
