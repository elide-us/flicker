...
/// Strip the `.pack.json` double extension (and the gz-at-rest `.gz` tail)
/// down to a display name.
pub fn pack_display_name(path: &Path) -> String {
    let name = path.file_name().and_then(|s| s.to_str()).unwrap_or("pack");
...
/// `Alpha/content/package/characters` is laid out), newest-name-sorted for a stable grid order.
