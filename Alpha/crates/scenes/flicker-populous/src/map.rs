...
pub const DEFAULT_FREQ: u32 = 96;

/// The standard tile width, miles — THE canon cell size (Aaron): it stays
/// constant as the planet scales, so frequency IS planet size.
pub const TILE_MI: f64 = 49.65;
/// The standard world's diameter, miles (Earth's mean) — what frequency 96
/// tiles out to at [`TILE_MI`] per cell.
pub const EARTH_DIAMETER_MI: f64 = 7_917.5;

/// The diameter a map of `freq` tiles out to, in miles: the tile stays the
/// standard width, so the planet scales linearly with frequency —
/// `EARTH_DIAMETER_MI · freq / DEFAULT_FREQ`.
pub fn diameter_mi(freq: u32) -> f64 {
    EARTH_DIAMETER_MI * f64::from(freq) / f64::from(DEFAULT_FREQ)
}
