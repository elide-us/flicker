//! flicker-csg: the first Alpha client, focused on CSG editing and cluster
...
/// Fixed studio lighting for the whole flicker-csg world: one steady
...
fn main() -> Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                "flicker_csg=info,flicker_app=info,flicker_render=warn".into()
            }),
        )
        .init();
    ...
    if let Some(arg) = std::env::args().nth(1) {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!("flicker-csg — flicker CSG editing / cluster manipulation demo");
                println!("Usage:");
                println!("  flicker-csg           run the demo (loads bake/ if present, else contours)");
                println!("  flicker-csg --bake    contour the 3×3 field and write bake/cluster_*.json, then exit");
                return Ok(());
            }
