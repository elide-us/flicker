...
struct SlotDef {
    /// The Model/result key shared with `scripts/hud.lua` (`slot_<key>`).
    key: &'static str,
    dir: &'static str,
    file: &'static str,
    fit: Fit,
...
struct Viewer {
    ...
    sheaths: Vec<Slot>,
    /// The Lua HUD (`scripts/hud.lua` + `UI.paperdoll`): inventory bar, view toggles, and
...
impl Scene for Viewer {
    fn enter(&mut self, renderer: &mut Renderer) {
        ...
        self.script = match ScriptHost::from_file(HUD_SCRIPT_PATH) {
            Ok(s) => {
                load_ui_json(&s, HUD_UI_ELEMENTS); // layout (`UI.paperdoll`)
                load_widgets(&s); // the shared immediate-mode toolkit
    ...
    fn update(&mut self, dt: Duration, input: &InputState, _renderer: &Renderer) -> Transition {
        ...
        // ── The Lua HUD owns every display toggle (scripts/hud.lua + `UI.paperdoll`): the
        ...
        if let Some(script) = self.script.as_ref() {
            ...
            match script.update(input, screen.x, screen.y) {
                Ok(res) => {
                    // The script reports whether the HUD owns the mouse — cursor over a
                    // panel, OR a slider drag in progress (which captures until release).
                    // That click must not ALSO pick the scene behind it, nor orbit the
                    // camera (user, 2026-07-16: dragging a slider was spinning the view).
                    let over_hud = res.is_on("hud_hit");
                    ...
                    if input.mouse_left_pressed && !over_hud && !self.last_globals.is_empty() {
                        ...
                        self.editing =
                            self.pick_slot(input.mouse_position, screen, world, &globals);
                ...
                Err(e) => tracing::error!("HUD update failed: {e}"),
            }
        }
    ...
    fn render(&mut self, renderer: &mut Renderer) {
        ...
        if let (Some(script), Some(white)) = (self.script.as_ref(), self.hud_white) {
...
fn build_viewer(base_dir: &Path, anim_dir: &Path) -> Viewer {
    ...
    let (sm, sm_states) = match state::load_pack(&anim_dir.join("Katanami.pack.json")) {
        Ok(def) => {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                "flicker_paperdoll=info,flicker_app=info,flicker_render=warn".into()
            }),
        )
        .init();

    // Content lives in the shared tree under Alpha/content/characters/. The BASE body is
    // PrismHumanBaseA — a Meshy AUTO-rigged female whose 24-bone Mixamo rig was renamed to
    // our canonical bone names (tools/blender/rename_meshy_to_canonical.py) and exported
    // with retarget=true, so the shared clips drive HER proportions (rotation-only) rather
    // than snapping her to Katanami's. It borrows the Katanami ANIMATION library (clips +
    // pack) by shared bone names; finger/twist tracks stay unresolved until slice 2.
    let base_dir =
        PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"), "/../content/characters/PrismHumanBaseA"));
    let anim_dir =
        PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"), "/../content/characters/katanami"));
    // The shell owns the front-end (splash → menu → settings/pause) + the run loop;
    // START loads the rig / clips / pack and launches the model viewer.
...
mod tests {
    ...
    fn skinned_garment_loads_and_binds_to_the_body() {
        ...
        let garment = PathBuf::from(concat!(
            ...
            "/../content/characters/musefit/Corset-Duster.skinned.json"
        ));
        if !garment.exists() {
    ...
    fn fit_gadget_is_absent_until_a_piece_is_selected() {
        let h = host();
        h.set_model(&fit_model(false)).unwrap();
        let res = h.update(&InputState::new(), 1920.0, 1080.0).expect("update");
        ...
        h.draw(1920.0, 1080.0).expect("draw must not error with no selection");
    }
    ...
    fn fit_gadget_reports_values_when_selected() {
        let h = host();
        h.set_model(&fit_model(true)).unwrap();
        let res = h.update(&InputState::new(), 1920.0, 1080.0).expect("update");
        // Every row the SHARED list declares must round-trip — a new row in the JSON that
        // the script silently ignores would be a dead control.
        for row in ui()["paperdoll"]["fit"]["rows"].as_array().unwrap() {
            let k = row["key"].as_str().unwrap();
            assert!(res.get(k).is_some(), "gadget must report the declared row '{k}'");
        }
        assert!(
            (res.number("fit_u").unwrap() - 44.79).abs() < 0.01,
            "an untouched slider must report the Model's value back unchanged, not snap"
        );
        let cmds = h.draw(1920.0, 1080.0).expect("draw");
        assert!(!cmds.is_empty(), "gadget draws a panel + rows");
    }
    ...
    fn hud_hit_is_true_over_panels_and_false_over_open_scene() {
        ...
        assert!(h.update(&over, w, hgt).unwrap().is_on("hud_hit"), "cursor over the fit panel");

        let mut open = InputState::new();
        open.mouse_position = Vec2::new(w * 0.5, hgt * 0.35); // open scene, above the bar
        assert!(!h.update(&open, w, hgt).unwrap().is_on("hud_hit"), "cursor over open scene");
    }
    ...
    fn record_button_fires_only_when_clicked() {
        ...
        assert!(h.update(&click, w, hgt).unwrap().is_on("fit_record"), "click on RECORD fires");

        let mut hover = click.clone();
        hover.mouse_left_pressed = false;
        assert!(!h.update(&hover, w, hgt).unwrap().is_on("fit_record"), "hover must not fire");
    }
    ...
    fn clicking_a_slider_focuses_it_and_focus_persists() {
        ...
        click.mouse_left_pressed = true;
        assert_eq!(
            focus_of(&h.update(&click, w, hgt).expect("update")),
            "fit_sx",
        ...
        h.set_model(&fit_model(true).with("fit_focus", "fit_sx")).unwrap();
        let mut away = InputState::new();
        away.mouse_position = Vec2::new(w * 0.5, hgt * 0.3);
        let res = h.update(&away, w, hgt).expect("update");
        assert_eq!(
    ...
    fn clicking_a_row_label_focuses_without_changing_its_value() {
        ...
        click.mouse_left_pressed = true;
        let res = h.update(&click, w, hgt).expect("update");
        assert_eq!(
            res.get("fit_focus").and_then(|v| match v {
                Value::Text(s) => Some(s.as_str()),
                _ => None,
    ...
    fn attack_button_fires_only_on_click_and_only_while_animating() {
        ...
        h.set_model(&hud_test_model(true, true).with("animate", true)).unwrap();
        let mut click = InputState::new();
        click.mouse_position = center;
        click.mouse_left_pressed = true;
        assert!(h.update(&click, w, hgt).unwrap().is_on("attack"), "click on Attack fires while animating");

        // Same click, but Pose mode → the button isn't even present, so no fire.
        h.set_model(&hud_test_model(true, true).with("animate", false)).unwrap();
        assert!(!h.update(&click, w, hgt).unwrap().is_on("attack"), "Attack must not fire in Pose mode");

        // Hover (no click) while animating → no fire.
        h.set_model(&hud_test_model(true, true).with("animate", true)).unwrap();
        let mut hover = InputState::new();
        hover.mouse_position = center;
        assert!(!h.update(&hover, w, hgt).unwrap().is_on("attack"), "hover must not fire");
    }
    ...
    fn record_confirmation_is_drawn_only_while_the_engine_publishes_one() {
        ...
        h.set_model(&fit_model(true).with("fit_status", "Saved 9 pieces -> musefit/fits.json"))
            .unwrap();
        let cmds = h.draw(w, hgt).expect("draw");
        assert!(has_text(&cmds, "Saved 9 pieces"), "the confirmation must be drawn");

        // Blank (the engine expires it) → gone, so a stale "saved" can't be misread as fresh.
        h.set_model(&fit_model(true).with("fit_status", "")).unwrap();
        let cmds = h.draw(w, hgt).expect("draw");
        assert!(!has_text(&cmds, "Saved"), "an expired confirmation must disappear");

        // Failures must surface too, not just successes.
        h.set_model(&fit_model(true).with("fit_status", "SAVE FAILED: permission denied"))
            .unwrap();
        let cmds = h.draw(w, hgt).expect("draw");
        assert!(has_text(&cmds, "SAVE FAILED"), "a failed write must be visible in-window");
    }
    ...
    fn a_slider_drag_captures_the_mouse_even_off_panel() {
        ...
        h.set_model(&fit_model(true)).unwrap();
        // Press on the Size slider.
        let mut press = InputState::new();
        press.mouse_position = Vec2::new(sx + sw_ * 0.5, row_y + g("slider_h") * 0.5);
        press.mouse_left_pressed = true;
        press.mouse_left = true;
        assert!(h.update(&press, w, hgt).unwrap().is_on("hud_hit"), "press on a slider captures");

        // Still HELD, cursor dragged far away over open scene — capture must persist.
        let mut drag = InputState::new();
        drag.mouse_position = Vec2::new(w * 0.3, hgt * 0.5);
        drag.mouse_left = true; // held, not a fresh click
        h.set_model(&fit_model(true)).unwrap();
        assert!(
            h.update(&drag, w, hgt).unwrap().is_on("hud_hit"),
        ...
        let mut release = InputState::new();
        release.mouse_position = Vec2::new(w * 0.3, hgt * 0.5);
        h.set_model(&fit_model(true)).unwrap();
        assert!(
            !h.update(&release, w, hgt).unwrap().is_on("hud_hit"),
    ...
    fn hud_script_runs_with_model() {
        ...
        load_ui_json(&host, HUD_UI_ELEMENTS); // layout (`UI.paperdoll`)
        load_widgets(&host);
        host.set_model(&hud_test_model(true, false)).expect("publish model");
        let input = InputState::new();
        let res = host.update(&input, 1920.0, 1080.0).expect("hud update");
        // Every slot reports back, so the engine can never miss one.
        for d in SLOTS {
            assert!(
                res.get(&format!("slot_{}", d.key)).is_some(),
                "slot '{}' must report its state back",
                d.key
            );
        }
        let cmds = host.draw(1920.0, 1080.0).expect("hud draw");
        assert!(!cmds.is_empty(), "hud emits panel + cell + toggle commands");
    }
    ...
    fn inventory_cell_toggles_only_when_a_piece_is_loaded() {
        ...
        // Centre of the first cell, derived the same way hud.lua lays the bar out.
        let ui: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(HUD_UI_ELEMENTS).unwrap()).unwrap();
        let inv = &ui["paperdoll"]["inventory"];
        let (slot, gap, pad, mb) = (
            inv["slot"].as_f64().unwrap() as f32,
            inv["gap"].as_f64().unwrap() as f32,
            inv["pad"].as_f64().unwrap() as f32,
            inv["margin_b"].as_f64().unwrap() as f32,
        );
        let n = SLOTS.len() as f32;
        let bar_w = n * slot + (n - 1.0) * gap;
        let x0 = (w - bar_w) * 0.5 - pad + pad; // panel.x + pad = first cell's x
        let y0 = h - mb - slot - pad + pad;

        let mut click = InputState::new();
        click.mouse_position = Vec2::new(x0 + slot * 0.5, y0 + slot * 0.5);
        click.mouse_left_pressed = true;

        let first = format!("slot_{}", SLOTS[0].key);
        host.set_model(&hud_test_model(true, false)).expect("model");
        let res = host.update(&click, w, h).expect("update");
        assert!(res.is_on(&first), "clicking a LOADED cell equips it");

        host.set_model(&hud_test_model(false, false)).expect("model");
        let res = host.update(&click, w, h).expect("update");
