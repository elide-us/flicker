...
/// Load the rig + clips (`flicker.rig` via `format::load_dir`) and the katana prop
/// from the bundled `assets/`, and build the viewer scene. The `.pack` state graph
/// is loaded inside [`Viewer::new`]. Panics with a clear message if the bundled
/// assets can't be read (a build/setup error, not a runtime condition) — the whole
/// point of this viewer is that it plays the *real* exported data.
fn build_viewer(assets: &Path) -> Viewer {
    let model = format::load_dir(assets).expect("load flicker.rig assets");
    tracing::info!(
    ...
    Viewer::new(model, assets.to_path_buf(), katana_mesh)
}
...
fn main() -> Result<()> {
    ...
    // Content lives in the shared tree, not inside the crate: the katanami character
    // bundle sits under Alpha/content/characters/ (a raw fbximport export — the
    // authored source, awaiting the content-processing pipeline that will split it
    // into clean rigs/ + clips/ + textures/ by-type assets).
    let assets = PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"), "/../content/characters/katanami"));
    ...
    flicker_shell::run(flicker_shell::ShellConfig {
        game_scene: Box::new(move || Box::new(build_viewer(&assets))),
    })
}
