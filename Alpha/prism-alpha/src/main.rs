...
mod tests {
    ...
    /// realm order: Populous (Game Master), the Developer benches, then Solar Birth +
    /// Click Trainer (Adventurer). DM stays dark until its bench migrates (backlog in
    ...
    fn roster_holds_the_migrated_benches() {
        ...
        assert_eq!(
            ...
            ["populous"],
            "Populous is the Game Master bench"
        );
        assert_eq!(
            realm_ids(REALM_DEVELOPER),
            ["componentcatalog", "quartermaster", "sablework"],
            "the Component Catalog + the Quartermaster + Sablework are the Developer benches"
        );
        assert_eq!(
            realm_ids(REALM_ADVENTURER),
            ["solarbirth", "clicktrainer"],
            "Solar Birth + Click Trainer are the Adventurer benches"
        );
        assert!(
