    ...
    vec![
        ...
        SceneEntry::new("godmode", "God Mode", "primary", flicker_godmode::scene)
            ...
            .with_info(SceneInfo::new(
                ...
                "Simulation",
                "World / Chemistry",
                "The world-simulation console — a living planet on its own thread: ten field views over the layer-shell globe, process gates to hold or release, a rack of physics levers, and the five-axis life-supporting verdict.",
                "Clay 0.1 · Bench · world console",
            )),
        SceneEntry::new("pocepochs", "Epoch Simulation", "primary", flicker_pocepochs::scene)
            .with_realm(REALM_GAMEMASTER)
            .with_info(SceneInfo::new(
                "Epoch Simulation",
                "Simulation",
                "World / Epochs",
                "The forward-regenerative planet sim — watch a fresh seed cool through the epochs, slice the layer stack open, and read the life-supporting conditions as they emerge.",
                "Clay 0.1 · Bench · epoch sim",
            )),
...
mod tests {
    ...
    /// realm order: Populous + God Mode + the Planet Simulation (Game Master), the
    /// Developer benches, then Solar Birth + Click Trainer + the Prism Test Room +
    /// the Controller Tester (Adventurer). DM stays dark until its bench migrates
    /// (backlog in MCP). Pins the set so a stray re-add of an un-migrated bench is caught.
    #[test]
    fn roster_holds_the_migrated_benches() {
        ...
        assert_eq!(
            ...
            ["populous", "godmode", "pocepochs"],
            "Populous, God Mode, and the Planet Simulation (Epoch Simulation) are the \
             Game Master benches — the realm where the world itself is authored"
        );
        assert_eq!(
